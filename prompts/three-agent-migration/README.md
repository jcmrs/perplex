...
|-------|-------|----------|----------|-------------|
| 1 | CDIR | Terminal-1 | 15-20 min | CDIR identity setup (first boot) |
| 2 | CDIR | Terminal-1 | 30-45 min | Configuration migration |
| 3 | CDIR | Terminal-1 | 45-60 min | Workspace coordination update |
| 4 | CDIR | Terminal-1 | 45-60 min | Automation & hooks update |
| 5 | CDIR | Terminal-1 | 60-90 min | Documentation update |
| 6 | CDIR | Terminal-1 | 30-45 min | GitHub workflows update |
| 7 | CDIR + CEXE | Both | 60-90 min | Validation & testing (CEXE first boot) |
| 8 | CDIR | Terminal-1 | 30-45 min | Production activation |
...
3. ✅ User is prepared to coordinate two local terminal windows
...
- **Terminal-1:** For CDIR (cli-claude-director-001)
- **Terminal-2:** For CEXE (cli-claude-executor-001)
- **Browser:** For Web (web-claude-designer-001) - Phase 9 only
- Working directory: `/home/user/perplex`
- Git access: Both terminals
...
3. Open Terminal-1 (will become CDIR)
4. Navigate to project: `cd /home/user/perplex`

### Step 2: Execute Phases 1-8 (CDIR and CEXE)

**Phase 1: CDIR Identity Setup (Terminal-1)**
```bash
cat prompts/three-agent-migration/PHASE-1-CDIR-IDENTITY-SETUP.md
```
...
**Phase 2: Configuration Migration (Terminal-1)**
```bash
cat prompts/three-agent-migration/PHASE-2-CONFIG-MIGRATION.md
```
...
**Phase 3: Workspace Coordination (Terminal-1)**
```bash
cat prompts/three-agent-migration/PHASE-3-WORKSPACE-COORDINATION.md
```
...
**Phase 4: Automation & Hooks (Terminal-1)**
```bash
cat prompts/three-agent-migration/PHASE-4-AUTOMATION-HOOKS.md
```

- CDIR updates all automation scripts
- CDIR updates git hooks for three agents
- **Validation gate:** Scripts work, hooks enforce

**Phase 5: Documentation (Terminal-1)**
```bash
cat prompts/three-agent-migration/PHASE-5-DOCUMENTATION.md
```
...
**Phase 6: GitHub Workflows (Terminal-1)**
```bash
cat prompts/three-agent-migration/PHASE-6-GITHUB-WORKFLOWS.md
```

- CDIR updates GitHub Actions workflows
- **Validation gate:** Workflows recognize three agents

**Phase 7: Validation & Testing (Terminal-1 + Terminal-2)**
```bash
cat prompts/three-agent-migration/PHASE-7-VALIDATION-TESTING.md
```

- CDIR pushes config branch
- CDIR creates test specification
- **User opens Terminal-2** (CEXE first boot)
...
**Phase 8: Production Activation (Terminal-1)**
```bash
cat prompts/three-agent-migration/PHASE-8-PRODUCTION-ACTIVATION.md
```
...
cp .claude/identity-web.json.backup .claude/identity-web.json
cp .claude/identity-cli.json.backup .claude/identity-cli.json
cp .claude/workspace-coordination.yml.backup .claude/workspace-coordination.yml
cp .claude/agent-registry.json.backup .claude/agent-registry.json

# Delete CDIR identity
rm .claude/identity-cli-director.json
...
# Announce rollback
echo "[From: Web] Migration rolled back. Two-agent architecture restored."
```
...
```bash
cat .claude/agent-registry.json | jq '.agents[] | {agent: .short_name, status: .status, work: .workspace.current_work}'
```
...
1. ✅ CDIR operational (Terminal-1)
2. ✅ CEXE operational (Terminal-2)
...
A: Rollback plan exists (restore backups, reset to main). See "Rollback" section.
...
A: CDIR (designer) and CEXE (executor) are separate agents needing independent sessions.
